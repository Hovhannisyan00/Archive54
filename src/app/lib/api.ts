...
export const getUserById = (id:string):Partial<IUser> => {
    return authDB
            .prepare("SELECT name, surname, password FROM users WHERE id = ?")
            .get(id) as Partial<IUser>
}



export const updateUserById = (user:any):Partial<IUser> => {
    return authDB
           .prepare("UPDATE users SET login = @login WHERE id =@id")
           .run(user) as Partial<IUser>
}

export const getAllUsername =()=>{
    return authDB
    .prepare("SELECT login FROM users")
    .all() as Partial<IUser>[]
}